...
assertByDeadlineIOCustom str deadline f = withFrozenCallStack $ do
  ...
  unless success $ do
    ...
    if currentTime < deadline
      then do
        liftIO $ IO.threadDelay 1_000_000
        assertByDeadlineIOCustom str deadline f
      else do
        error $ "Condition not met by deadline: " <> str
...
assertExpectedSposInLedgerState output (NumPools numExpectedPools) execConfig = withFrozenCallStack $ do
  ...
  ePoolSet <-  liftIO (Aeson.eitherDecodeFileStrict' @(Set PoolId) output)
  case ePoolSet of
    Left err -> error $ "Failed to decode stake pools from ledger state: " <> err
    Right poolSet -> do

       let numPoolsInLedgerState = Set.size poolSet
       unless (numPoolsInLedgerState == numExpectedPools) $
         error $ "Expected number of stake pools not found in ledger state. Expected: "
              <> show numExpectedPools <> ", Actual: " <> show numPoolsInLedgerState
