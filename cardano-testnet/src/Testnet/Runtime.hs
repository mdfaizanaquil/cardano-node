...
import           Testnet.Process.NewRun (procNode)
...
startNode tp node ipv4 port _testnetMagic nodeCmd = GHC.withFrozenCallStack $ do
  ...
  --  H.assertM $ 
    isClosed <- liftIO $ Ping.waitForPortClosed 30 0.1 port
    case isClosed of
      False -> error $ "Port  is still in use after 30 seconds before starting node"
      True -> pure ()
...
startLedgerNewEpochStateLogging testnetRuntime tmpWorkspace = withFrozenCallStack $ do
  ...
  liftIO $ IO.doesDirectoryExist logDir >>= \case
    True -> pure ()
    False -> do
      error "Fail"
  ...
  liftIO $ IO.doesFileExist logFile >>= \case
    True -> do
      return () 
...
asyncRegister_ :: HasCallStack
               ...
               -> m ()
asyncRegister_ act = GHC.withFrozenCallStack $ void $ allocate (H.async act) cleanUp
  where
